...
void *grabRsrcs(void *threadid)
{
   if((int)threadid == THREAD_1)
   {
     ...
     while(ret != 0) {
        ...
        backoff.tv_nsec = rand() % 1000000 + 1000;
        printf("THREAD 1 getting B with timed lock %ld\n", backoff.tv_nsec);
        ret = pthread_mutex_timedlock(&rsrcB, &backoff);
        if (ret != 0) {
            rsrcACnt -= 1;
            pthread_mutex_unlock(&rsrcA);
            printf("THREAD 1 could not get B, released A\n");
        }
     }
     rsrcBCnt++;
   ...
   else
   {
     ...
     while(ret != 0) {
        ...
        if(!noWait) usleep(1000000);
        backoff.tv_nsec = rand() % 1000000 + 1000;
        printf("THREAD 2 getting A with timed lock %ld\n", backoff.tv_nsec);
        ret = pthread_mutex_timedlock(&rsrcA, &backoff);
        if (ret != 0) {
            rsrcBCnt--;
            pthread_mutex_unlock(&rsrcB);
            printf("THREAD 2 could not get A, released B");
        }
     }
     rsrcACnt++;
